...
function mapRow(s: StoryListItem, index: number): Row {
    const viewsK = Math.max(1, Math.round(s.readTime * 21 + s.commentCount * 14));
    return {
        id: s.id,
        rank: index + 1,
        tag: s.sport,
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (res.success && res.data) {
                setRows(res.data.items.map((s, i) => mapRow(s, i)));
            } else setRows([]);
            setLoading(false);
        })();
        return () => {
                            ...
                            {items.map((n, i) => (
                                <article key={n.id} style={{ animationDelay: `${i * 70}ms` }} className="group flex items-center gap-6 p-6 rounded-2xl bg-card border border-border hover:border-primary hover:shadow-glow transition-smooth cursor-pointer animate-slide-in-right">
                                    ...
                                    <div onClick={(e) => e.stopPropagation()}>
                                        <BookmarkButton item={{ id: n.id, title: n.title, href: `/article/${n.slug}`, sport: n.tag }} />
                                    </div>
                                </article>
                            ))}
